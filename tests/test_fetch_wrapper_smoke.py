...
ApiController spawns to fetch Scalable Capital data.

These DO NOT hit the real Scalable API. They verify that the wrapper:
  • parses CLI flags correctly (--email, --data-dir, --full)
  • returns an argparse error when required args are missing
  • handles the --full flag
  • imports cleanly (no syntax errors)

Run with:  python3 -m unittest discover -s tests/

Uses stdlib unittest only — no pytest, no extra deps. Mirrors the
TR/GBM owncloud smoke tests (TRIO-PLAYBOOK process gates).
"""
import os
import subprocess
import sys
...
# Exit codes from ScalableService.php / BaseOwnCloudService.php — keep in sync.
EXIT_OK = 0
...
class TestFetchWrapperSmoke(unittest.TestCase):
    """Sanity checks that don't talk to the real Scalable API."""

    def test_wrapper_file_exists(self):
        self.assertTrue(WRAPPER.exists(), f'fetch_wrapper.py not found at {WRAPPER}')

    def test_wrapper_help_without_python_crash(self):
        """--help must not crash with a Python error (covers syntax + imports)."""
    ...
    def test_missing_required_args_returns_argparse_error(self):
        """Without --email / --data-dir, argparse exits 2."""
    ...
    def test_full_flag_recognized(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            rc, _out, err = run_wrapper(
                ['--email', 'nobody@example.com', '--data-dir', tmp, '--full'],
                timeout=5,
            )
    ...
    def test_exit_codes_match_php(self):
        """Every EXIT_* constant here should appear in ScalableService.php or
        its BaseOwnCloudService parent (where the shared block lives)."""
        text = ''
        for fname in ('ScalableService.php', 'BaseOwnCloudService.php'):
        ...
        for name in ('EXIT_OK', 'EXIT_MFA_REQUIRED', 'EXIT_MFA_INVALID',
            ...
            self.assertIn(name, text,
                          f'{name} missing from ScalableService.php / '
                          'BaseOwnCloudService.php — wrapper/PHP drift')

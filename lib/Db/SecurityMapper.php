...
class SecurityMapper extends Mapper {
	public function __construct(IDBConnection $db) {
		parent::__construct($db, 'sc_securities', Security::class);
	}

	/** @return Security[] */
	public function findByUser(string $userId): array {
		$sql = 'SELECT * FROM `*PREFIX*sc_securities` WHERE `user_id` = ?';
	...
	public function findByExtId(string $userId, string $extId): ?Security {
		$sql = 'SELECT * FROM `*PREFIX*sc_securities` WHERE `user_id` = ? AND `ext_id` = ?';
		try {

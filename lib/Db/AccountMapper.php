...
class AccountMapper extends Mapper {
	public function __construct(IDBConnection $db) {
		parent::__construct($db, 'sc_accounts', Account::class);
	}

	/** @return Account[] */
	public function findByUser(string $userId): array {
		$sql = 'SELECT * FROM `*PREFIX*sc_accounts` WHERE `user_id` = ?';
		return $this->findEntities($sql, [$userId]);
	}

	public function findByKey(string $userId, string $accountKey): ?Account {
		$sql = 'SELECT * FROM `*PREFIX*sc_accounts` WHERE `user_id` = ? AND `account_key` = ?';
		try {
	...
	public function deleteByUser(string $userId): void {
		$this->execute('DELETE FROM `*PREFIX*sc_accounts` WHERE `user_id` = ?', [$userId]);
	}
}

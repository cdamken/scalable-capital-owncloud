...
class TransactionMapper extends Mapper {
	public function __construct(IDBConnection $db) {
		parent::__construct($db, 'sc_transactions', Transaction::class);
	}

	/** @return Transaction[] */
	public function findByUser(string $userId): array {
		$sql = 'SELECT * FROM `*PREFIX*sc_transactions` '
	...
	public function findByExternalId(string $userId, string $externalId): ?Transaction {
		$sql = 'SELECT * FROM `*PREFIX*sc_transactions` WHERE `user_id` = ? AND `external_id` = ?';
		try {
